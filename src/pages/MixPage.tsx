...
export function MixPage() {
  ...
  return (
    <MobileShell withNav className="learning-page mix-page">
      ...
      <button className="primary-button mix-generate" type="button" onClick={async () => { const tutorial = await learningService.generateMix(selection); navigate('/tutorial', { state: { from: '/mix', tutorialId: tutorial.id } }); }}><WandSparkles size={18} />生成教程</button>
      <BottomNav />
    </MobileShell>
  );
}
