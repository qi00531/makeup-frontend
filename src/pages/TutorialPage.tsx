...
export function TutorialPage() {
  ...
  useEffect(() => { void learningService.getTutorial().then(setTutorial); }, []);
  ...
  return (
    <MobileShell className="learning-page tutorial-page">
      ...
      <section className="step-detail-card"><span className="step-number">STEP {String(step.order).padStart(2, '0')}</span><h2>{step.title}</h2>{mode === 'beginner' && <p>{step.instruction}</p>}<div className="product-row"><span><PackageOpen size={17} /></span><div><small>使用产品</small><strong>{step.product}</strong></div><i style={{ backgroundColor: step.color }} /></div>{mode === 'beginner' && <div className="expert-tip"><Sparkles size={14} /><span>{step.expertTip}</span></div>}<button className="slice-button" type="button"><Film size={16} /><span>查看原视频切片</span><small>{step.videoSlice}</small><ChevronRight size={15} /></button>{step.hasEyeGuide && <Link className="eye-guide-link" to="/eyes"><Eye size={17} />查看眼部精讲<ChevronRight size={16} /></Link>}</section>
    </MobileShell>
  );
}
